...
def load_models_and_data():
    ...
    if missing:
        raise FileNotFoundError(
            ...
            "تأكد إنها مرفوعة على GitHub في نفس فولدر app.py."
        )

    df_data = pd.read_csv("hatla2ee_cleaned_data.csv")

    # تنظيف الأعمدة النصية الأساسية عشان نمنع أخطاء NaN لاحقًا
...
if search_btn:
    ...
    if uploaded_file is not None:
        try:
            ...
            st.error("⚠️ مقدرتش أفتح الصورة المرفوعة. جرب صورة JPG/PNG سليمة.")
            pil_img = None
    ...
    if top_results.empty:
        st.warning("⚠️ مفيش نتائج مطابقة. جرب تغيّر شروط البحث.")
        st.stop()
    ...
    if missing_features:
        ...
        pred_log = full_pricing_pipeline.predict(top_results[pred_features])
